...
(function ($$, $, window, document, undefined) {
    /**
     * Module '$$.utils' in root namespace.
     * @module {$$.utils}
     */
    $$.utils = $$.utils || {};
    ...
    $$.utils.htmlList = function (type) {
        if (!type || ($$.utils.typeOf(type) != 'string') || ((type.toLowerCase() != 'u') && (type.toLowerCase() != 'o')) || (arguments.length <= 1)) return false;
        var result = "<" + type.toLowerCase() + "l><li>";
        var args;
        if ($$.utils.typeOf(arguments[1]) === 'array') args = arguments[1];
     ...
     * @copyright MIT licence.
     *******************************************************************************/
    $$.utils.debug = function (obj, maxDepth) { //construct with argument
        var tree = [],
            refs = [],
            max;
        refs.push([0, obj]);
        if (maxDepth && ($$.utils.typeOf(maxDepth) === 'number') && (maxDepth > 0)) max = maxDepth;
        else max = -1;

        function traverse(obj, i) {
            var value, str = '',
                tmp = '',
                circular, found, iterate = false;
            for (var property in obj) {
                ...
                if (value === null) {
                    tmp = 'null';
                    found = true;
                } else if ($$.utils.typeOf(value) === 'undefined') {
                    tmp = 'undefined';
                    found = true;
                } else if (value === '') {
                    tmp = '\'\'';
                    found = true;
                } else if ($$.utils.typeOf(value) === 'boolean') {
                    tmp = (value) ? '\'true\'' : '\'false\'';
                    found = true;
                } else if ($$.utils.typeOf(value) === 'string') {
                    tmp = '\'' + value + '\'';
                    found = true;
                } else if ($$.utils.typeOf(value) === 'number') {
                    tmp = value;
                    found = true;
                ...
                if (found) {
                     ...
                     * --------------------
                     */
                } else if (($$.utils.typeOf(value) === 'function') || ($$.utils.typeOf(value) === 'object')) {
                    circular = false;
                    ...
                    if (!circular) {
                        if ($$.utils.typeOf(value) === 'object') {
                            ...
                            tmp = '\'object\'';
                        } else if ($$.utils.typeOf(value) === 'function') tmp = value;
                    }
                    str[2] = tmp;
                    tree[tree.length - 1] = str;
        ...
        return {
            ...
            formatRow: function (arr) {
                var quotes = ['\'object\'', '\'function\'', 'circular reference'],
                    pos = -1,
                    circular = false;
                if (arr[0] == -1) return '<span style="color: red">' + arr.join(', ') + '</span>';
                for (var i = 0; i < quotes.length; i++) {
                    pos = (arr[2] + '').lastIndexOf(quotes[i]);
                    if (pos >= 0) {
                        if (i === 2) circular = true;
                        break;
                    }
                }
                if (pos >= 0) arr[2] = '<span style="color: ' + ((circular) ? 'red' : 'blue') + '">' + arr[2] + '</span>';
                return arr.join(', ');
            },
            popup: function (tag) {
                ...
                if ((tag) && (tag.toLowerCase) && ((tag.toLowerCase() === 'u') || (tag.toLowerCase() === 'o'))) {
                    ...
                    w.document.writeln($$.utils.htmlList(tag, list));
                }
